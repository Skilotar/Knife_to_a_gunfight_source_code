...
namespace Knives
{
    ...
    public class Queen : GunBehaviour
    {
        ...
        public IEnumerator nytraPlacerCtrl(PlayerController Owner)
        {
            Projectile projectile2 = ((Gun)ETGMod.Databases.Items[15]).DefaultModule.projectiles[0];
            GameObject gameObject = SpawnManager.SpawnProjectile(projectile2.gameObject, Owner.CurrentGun.sprite.WorldCenter, Quaternion.Euler(0f, 0f, (Owner.CurrentGun == null) ? 0f : Owner.CurrentGun.CurrentAngle), true);
            ...
            AIActor orLoadByGuid = EnemyDatabase.GetOrLoadByGuid("c0260c286c8d4538a697c5bf24976ccf");
            AIActor aiactor;
            IntVector2 aim = Vector2Extensions.ToIntVector2(component.LastPosition, VectorConversions.Round);
            RoomHandler room;
            room = GameManager.Instance.Dungeon.data.GetAbsoluteRoomFromPosition(aim);

            if (room != null && room == Owner.CurrentRoom && Owner.IsInCombat && Owner.unadjustedAimPoint.GetAbsoluteRoom() == Owner.CurrentRoom)
            {
                aiactor = AIActor.Spawn(orLoadByGuid.aiActor, component.LastPosition, room, true, AIActor.AwakenAnimationType.Default, true);
                PhysicsEngine.Instance.RegisterOverlappingGhostCollisionExceptions(aiactor.specRigidbody, null, false);
                aiactor.CanTargetEnemies = true;
                aiactor.CanTargetPlayers = false;
                aiactor.IsHarmlessEnemy = true;
